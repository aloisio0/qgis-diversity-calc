...
def dc_summarizePoly(poly, lyrPoint, fldSpecies):
    ...
    #                     of the species
    #    The purpose of the funcition is to sumerize the number of
    #    observation of each species inside the polygon in the form
    #    of a dicitionary containing species as keys and the number of
    #    observations as values.

    dctPoly = {}
    # loop through all the points the interset the polygon bounding box
    for obs in lyrPoint.getFeatures(poly.geometry().boundingBox()):
        ...
        if poly.geometry().contains(obs.geometry()):
            ...
            if sSpecies in dctPoly.keys():
                # if it done, increase the count to 1
                dctPoly[sSpecies] += 1
            else:
                # if there is no entry for the species, create it and set its initial value 1
                dctPoly[sSpecies] = 1
    return dctPoly
...
def dc_mergeDictionaries(dMain, cat, dPoly):
    ...
    #          cat   = a string conatining the name of the category to be merged
    ...
    #      the apropriate summary information in dMain.
